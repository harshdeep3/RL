...
class MT5Class:
    ...
    def get_acc_info(self) -> None:
        ...
        if mt5.account_info() is None:
            self.logger.error("Account info is None!")
        else:
            self.account_info = mt5.account_info()._asdict()

    def get_all_orders(self) -> int:
        """
        ...
        if orders > 0:
            self.logger.debug(f"Total order = {orders}")
        else:
            self.logger.debug(f"No orders")
    ...
    def get_orders_total_by_symbol(self, symbol: str = "USDJPY") -> int:
        ...
        if len(orders) > 0:
            self.logger.debug(f"{symbol} orders = {orders}")
        else:
            self.logger.debug(f"No orders for {symbol}")

        return orders

    def get_symbol_info(self, symbol: str = "USDJPY") -> object:
        """
        ...
        if not symbol_info.visible:
            self.logger.debug(f"{symbol} is not visible")
            return None

        return symbol_info

    def buy_symbol(self, symbol: str, price: float, lot: float, sl: float, tp: float, deviation: int) -> dict:
        """
        ...
        self.logger.debug(f"1. order_send(): by {symbol} {lot} lots at {price} with deviation={deviation} points")

        return result

    def close_buy_position(self, symbol: str, price: float, lot: float, deviation: int, position_id: int) -> dict:
        """
        ...
        # check the execution result
        self.logger.debug(f"close position #{position_id}: sell {symbol} {lot} lots at {price} with deviation={deviation} points")

        return result

    def close_sell_position(self, symbol: str, price: float, lot: float, deviation: int, position_id: int) -> dict:
        """
        ...
        # check the execution result
        self.logger.debug(f"close position #{position_id}: buy {symbol} {lot} lots at {price} with deviation={deviation} points")

        return result

    def sell_symbol(self, symbol: str, price: float, lot: float, sl: float, tp: float, deviation: int) -> dict:
        """
        ...
        # check the execution result
        self.logger.debug(f"order_send(): by {symbol} {lot} lots at {price} with deviation={deviation} points")

        return result

    def result_check(self, result: object) -> None:
        """
        ...
        if result.retcode != mt5.TRADE_RETCODE_DONE:
            self.logger.error(f"2. order_send failed, retcode={result.retcode}")
            # request the result as a dictionary and display it element by element
            result_dict = result._asdict()
            for field in result_dict.keys():
                self.logger.error(f"  {field}={result_dict[field]}")
                # if this is a trading request structure, display it element by element as well
                if field == "request":
                    traderequest_dict = result_dict[field]._asdict()
                    for tradereq_filed in traderequest_dict:
                        self.logger.error(f"  traderequest: {tradereq_filed}={traderequest_dict[tradereq_filed]}")


    def get_historic_data(self, fx_symbol: str, fx_timeframe: int, fx_count: int) -> pd.DataFrame | None:
        """
...
if __name__ == "__main__":
    ...
    # get data
    df = mt5_obj.get_historic_data(symbol, timeframe, count)
